...
class Agent:
    ...
    def get_all_rules(self):
        rules = []
        for category in self.grammar.values():
            rules += category
        return rules

    def get_sub_rule(self, label, meaning):
        sub_rule = None
        if label in self.grammar.keys():
            for rule in self.grammar[label]:
    ...
    def get_all_sub_rules(self):
        rules = []
        for label in self.grammar.keys():
            if label != "S":
    ...
    def add_rule(self, rule):
        ...
        if rule.label in self.grammar.keys():
            self.grammar[rule.label] += [rule]
        else:
            self.grammar[rule.label] = [rule]
    ...
    def remove_rule(self, rule):
        ...
        if rule.label in self.grammar.keys():
            self.grammar[rule.label].remove(rule)
    ...
    def relabel(self, old_label, new_label):
        ...
        if old_label in self.grammar.keys():
            category = self.grammar.pop(old_label)
            self.grammar[new_label] = category
            for rule in self.grammar[new_label]:
